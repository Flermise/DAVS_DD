...
from SpiderCCGP.items import GZGGItem, PageUrlItem, FBLBGGItem
...
class GZMySQLPipeline(object):
    ...
    def do_GZGG_insert(self, cursor, item):
        insert_sql = """
                                ...
                                """
        cursor.execute(insert_sql, (item['project_num'], item['project_name'], item['items'], item['url'], item['soft_hard'], item['unit'], item['regions'],
                                                 item['annou_time'], item['first_annou_time'], item['correct_time'], item['project_contact'], item['project_phone'], item['unit_address'],
                                                 item['unit_contact_infor'], item['agent_name'], item['agent_address'], item['agent_contact'], item['text_path'], item['file_save_path']))
    ...
    def do_ZBGG_insert(self, cursor, item):
        ...
        item['total_money'], item['project_contact'], item['project_phone'], item['unit_address'], item['unit_contact_infor'],
        item['agent_name'], item['agent_address'], item['agent_contact'], item['text_path'], item['file_save_path']))
    def process_item(self, item, spider):
        """
...
#                             VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
#                             """
#             self.cursor.execute(insert_sql, (item['project_num'], item['project_name'], item['items'], item['url'], item['soft_hard'], item['unit'], item['regions'],
#                                              item['annou_time'], item['first_annou_time'], item['correct_time'], item['project_contact'], item['project_phone'], item['unit_address'],
#                                              item['unit_contact_infor'], item['agent_name'], item['agent_address'], item['agent_contact'], item['text_path'], item['file_save_path']))
#             self.conn.commit()
#         elif isinstance(item, PageUrlItem):
#             self.cursor.execute(
#                 "insert into page_url(url,type,kind) values('{0}','{1}','{2}')".format(item['url'], item['ptype'],item['kind']))
#             self.conn.commit()
#         return item
#
